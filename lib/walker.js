...
function Walker (options) {
  ...
  this.onWalk = function () {
    ...
    function next () {
      ...
      debug('next', (typeof path, path ? path.url : undefined), (typeof path, path ? path.url : undefined));
      if (!path) return self.emit('done');
      self.emit('walking', path.url, path.ua);
      path(function evaluate (err, zombie, status) {
        debug('evaluate', err, typeof zombie, status);
        if (err) self.emit('error', err, path.url, path.ua, zombie, status);
        self.emit('walked', path.url, path.ua, zombie, status);
        next();
      });
    }
    next();
